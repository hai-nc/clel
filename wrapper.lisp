...
(cl:defparameter *wrap-many-symbols-p* nil
  ...
  satisfying the Elisp predicate `clel-wrappable-symbol-p'. This variable is
...
NEWSYM: assign a new symbol name instead of the wrapped Elisp symbol."
  (cond
    ...
    ((and (symbolp elsym)
          ;; no clobber
          (let* ((elsym-in-clel (cl:find-symbol (symbol-name elsym) *clel-package*)))
            (or (null elsym-in-clel) ; either elsym-in-clel has not yet been created
                (null (fboundp elsym-in-clel) ; or it has not yet been fbound
                                                ; or macro-bound
                          )
     ...
     (let* ((def-fun (if t
			             'cl:defmacro
                         'cl:defun))
            (newsym (intern (symbol-name (or newsym elsym)) *clel-package*)))
       ;; (cl:shadow newsym pkg)
       (cl:export newsym *clel-package*)
...
(eval-string
 #str[(progn
        (defun clel-wrappable-symbol-p (item)
          (and (symbolp item)
               (or (macrop item)
                   (functionp item)
                   (subrp item))
			   (null ; exclude symbols with -- (internal functions/macros) and
                                        ;   those with ":" (which chokes Lisp as it already use 'clel
                                        ;   as the package of the wrapping symbol):
				(string-match-p  "[^-[:alnum:]]\\|--\\|:"
						         (symbol-name item)))))
        
        (defun clel-lisp-eval-string (str)
          (slime-eval `(cl:eval (cl:read-from-string ,str))))
        
        (defun clel-lisp-wrap-symbols-maybe (syms &optional noclobber)
          (require 'cl-lib)
          (let* (sym-subset)
            ...
            (while syms
                   ...
                   (slime-eval `(cl:let ((cl:*package* (cl:find-package 'clel)))
		                          (clel/1::wrap ',sym-subset nil ,noclobber))))))

        (add-hook 'after-load-functions
                  (defun clel--after-load-function (fullname)
...
FULLFNAME: the absolute filename of the library just loaded."
                    (let* (result)
                      (dolist (item
                               ...
                               (clel-lisp-wrap-symbols-maybe result t))
                        (and (clel-wrappable-symbol-p item)
                             (push item result)))))
                  t)
        )])
...
(cl:when t
  (eval-string
   ...
   (let ((my-macros nil)
         ...
         (my-other nil)
         name)
     (defun clel-print-list-full (lis &optional macrop)
...
to print a list in full."
     ...
     (do-all-symbols (sym)
       ...
       (cond
         ...
         (t (push sym my-other))))
     ;; ;; test
     ;; (clel-print-list-full "special-forms" my-special-forms)

     (with-temp-file "import.lisp"
       ;; (insert (format "(cl:in-package :%s)\n" 'clel))
       ;; (insert (format "(named-readtables:in-readtable :%s)\n" 'clel))
       (insert (format "(cl:setq %s::%s %S)\n" 'clel '*emacs-version* emacs-version))
       (clel-print-list-full my-macros t)
       (clel-print-list-full my-functions)
       (clel-print-list-full my-special-forms)
       (clel-print-list-full my-subrs)
       ;; (clel-print-list-full my-variables)
       ;; (clel-print-list-full my-other)
       ))]))

(cl:when nil
  (eval `(progn
           (setq clel-wrap-many-symbols-p ',clel/1::*wrap-many-symbols-p*
                 clel-wrap-these-symbols-also ',clel/1::*wrap-these-symbols-also*)
          ;; wrap these important symbols first:
          (clel-lisp-wrap-symbols-maybe clel-wrap-these-symbols-also t)))
  (eval-string
   #str[(let* ((regexp
                 (slime-eval `(cl:format nil "\\(^\\(~{~(~A~)~^\\|~}\\)\\|~(~A~)\\)"
                                         ...
                                         '-mode)))
               result)
          (do-all-symbols (item (clel-lisp-wrap-symbols-maybe result t))
			(and
             (clel-wrappable-symbol-p item)
             ;; - alternatively, one can wrap only a few symbols are needed for
             ;;   my init, so only a handful of symbols are selected here:
             (or clel-wrap-many-symbols-p (string-match-p regexp (symbol-name item)))
             (push item result))))]))
