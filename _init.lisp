...
(eval '(fringe-mode '(6 . 4)))

(eval '(dolist (param '(fullscreen)) ; width height background-color foreground-color
        (add-to-list 'frame-inherited-parameters param t)))

(eval '(progn ; ensure eval synchronously so that `my-frame-maximize' exists
...
(elshim/1:eval-string "(defmacro endless/define-conditional-key (keymap key def
                                                   &rest body)
...
(eval '(endless/define-conditional-key prog-mode-map (kbd "<tab>")
               'hs-hide-level
               (eq ?\( (char-after))))

;; ** clipmon-mode
(eval '(when (fboundp 'clipmon-mode)
...
;;   (interactive (list (elshim:read-file-name "Document: ")))
...
(cl:defparameter *bibfiles* (cl:list (cl:namestring (cl:probe-file "~/db.bib"))))
(cl:defparameter *bibpdfs* (cl:list (cl:namestring (cl:probe-file "~/pdf/"))))
...
(eval
 `(setq bibtex-completion-bibliography ',*bibfiles*
        ...
        bibtex-completion-library-path ',*bibpdfs*
